...
def processar_guia(numero_guia, numero_proad, nome_analista):
    try:
        ...
        caminho_planilha = r'\\10.0.20.65\Financeira\\CAJ\Automação - Fiança\Restituição - Planilha automação.xlsx'
        ...
        main_dir = r"C:\Users\flcarneiro\Documents\Alvara"
        analyst_folder_path = os.path.join(main_dir, str(nome_analista))

        # Cria a pasta do analista se não existir
        if not os.path.exists(analyst_folder_path):
            os.makedirs(analyst_folder_path)
            print(f"Criou a pasta do analista '{analyst_folder_path}'.")

        # Cria a subpasta do número PROAD dentro da pasta do analista
        new_folder_path = os.path.join(analyst_folder_path, str(numero_proad))

        # Cria a subpasta se não existir
        if not os.path.exists(new_folder_path):
            os.makedirs(new_folder_path)
            print(f"Criou a subpasta '{new_folder_path}'.")
